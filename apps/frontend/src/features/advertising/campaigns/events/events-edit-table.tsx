...
import { BiggerOnHover } from '@/features/advertising/advertising.styles';
import { EventColumns } from '@/features/advertising/campaigns/events/events-table';
...
const columnHelper = createColumnHelper<EventColumns>();

const EditTable = observer((props: { events: Array<EventColumns>; setNewEvents: any }) => {
	const { events, setNewEvents } = props;

	const [data, setData] = useState<EventColumns[]>([
		...events,
		{
			tracker: '',
			admin: '선택',
	...
	const TableTrackerInput = (info: CellContext<EventColumns, string>) => {
	...
	const TableMediaInput = (info: CellContext<EventColumns, string>) => {
		const onChange = (e: { target: { value: any } }) => {
			(info.table.options.meta as EditTableMeta).updateData(
				info.row.index,
				info.column.id,
	...
	const TableSelect = (info: CellContext<EventColumns, string>) => {
	...
	const TableIcon = (info: CellContext<EventColumns, number>) => {
		const onChange = () => {
	...
	const TableDeleteButton = (info: CellContext<EventColumns, unknown>) => {
		const onConfirm = () => {
