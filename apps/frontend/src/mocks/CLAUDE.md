...
- `handlers.ts` — 목 응답 정의. backend에 아직 없는 경로만 목킹한다 — `PATCH /advertising/:id`(상태 토글). (캠페인 활성 토글은 실제 `PATCH /campaigns/:id`, 예약 변경 화면은 실제 `/campaigns`·`/reservations` API로, 개발자 화면은 실제 `/users` API로 연동됨)
