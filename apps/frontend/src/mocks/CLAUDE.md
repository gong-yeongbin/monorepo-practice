...
- `handlers.ts` — 목 응답 정의. backend에 아직 없는 경로만 목킹한다 — `PATCH /advertising/:id`(상태 토글), `PATCH /campaigns/:id/block`, `/reservation/*` 3종(off 조회·생성·삭제 — on 조회는 실제 `GET /campaigns` 연동으로 대체됨), `POST /users`.
