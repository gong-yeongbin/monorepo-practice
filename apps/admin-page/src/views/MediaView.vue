...
const loadCampaignData = async () => {
  if (advertisingId.value && token.value && startDate.value && endDate.value) {
    ...
    const selectedCampaign = data.advertising.campaign.find(
      (cp: { token: string }) => cp.token === token.value,
    )

    if (selectedCampaign && selectedCampaign.dailyStatistic) {
      // 같은 날짜의 데이터를 합치기
      dailyStatistics.value = mergeDailyStatistics(selectedCampaign.dailyStatistic)
    } else {
