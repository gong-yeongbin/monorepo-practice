...
export class PostbackConsumerUseCase implements OnModuleInit {
	...
	private readonly consumer: Consumer = this.kafka.consumer({ groupId: 'mecross-system-postback' });
	...
	async onModuleInit() {
		...
		await this.consumer.subscribe({ topic: 'postback', fromBeginning: false });
