...
export class DailyUseCase {
	...
	async execute(dto: DailyDto, scope: AdvertisingScope): Promise<DailyRow[]> {
		return this.dashboardRepository.daily({ start_date: new Date(dto.start_date), end_date: new Date(dto.end_date) }, dto.token, scope);
	}
}
