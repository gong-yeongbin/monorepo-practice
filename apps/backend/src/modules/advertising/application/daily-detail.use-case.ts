// token 기준 일별 상세 통계를 조회하는 use-case
...
export class DailyDetailUseCase {
	...
	async execute(dto: DailyDto): Promise<DailyRow[]> {
		return this.advertisingRepository.dailyDetail(dto.token, { start_date: new Date(dto.start_date), end_date: new Date(dto.end_date) });
	}
}
